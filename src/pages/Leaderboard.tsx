...
type MetricType = 'kills' | 'deaths' | 'balance' | 'playtime' | 'blocks_broken' | 'mob_kills' | 'votes';
...
export default function Leaderboard() {
  ...
  const categories = [
    ...
    { id: 'balance' as MetricType, name: 'Money', icon: Coins, color: 'from-emerald-500 to-teal-600' },
  ...
  const handleRunSync = async () => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        setLastSyncMsg(json.message || 'Sync pass completed with 0 updates.');
      }
    } catch (err: any) {
      setLastSyncMsg(`Sync error: ${err.message}`);
    } finally {
      setSyncing(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto relative z-10 space-y-8">
        ...
        <div className="text-center space-y-4">
          <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-xs font-bold tracking-widest uppercase">
            ...
            <span>AJLB External Supabase Sync Engine</span>
          </div>
          ...
          <p className="text-slate-400 text-sm md:text-base max-w-2xl mx-auto">
            Live rankings synchronized from the ajLeaderboards MySQL database directly to Supabase.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            >
              ...
              <span>{syncing ? 'Syncing...' : 'Trigger Sync Pass'}</span>
            </button>
          ...
          {lastSyncMsg && (
            <p className="text-xs text-emerald-400 font-mono animate-fade-in">{lastSyncMsg}</p>
          )}
        </div>
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex items-center space-x-2 px-4 py-3 rounded-2xl border text-xs font-bold transition-all shrink-0",
                  isActive
            ...
            <div className="flex items-center space-x-3 w-full md:w-auto">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  {connected ? 'Live MySQL Data Connected' : 'Supabase Synchronized View'}
                </p>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-800/40 text-sm">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={3} className="py-12 text-center text-red-400 text-xs font-bold">{error}</td>
                  </tr>
